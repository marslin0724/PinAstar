...
void A_star_2_stack_OSC(MATRIX<__int8> &Sorted_G, MATRIX<double> &Metric_Table, vector<__int8> &Hard_RX, NODE_PATH &Node, NODE_PATH &Pre_Best_Goal, size_t pc_i, DECODING_INFO &decoding_info)
{
	size_t
		message_length(Sorted_G.Row_number),
		codeword_length(Sorted_G.Col_number);
	...
	do {
		...
		if ((Pointer.level == message_length) && (Pointer.D_z <= pc_i)) {
			...
			A_star_1_stack_OSC(Sorted_G, Metric_Table, Hard_RX, Pointer, Best_Goal_2, pc_i+2, decoding_info);
...
void A_star_3_stack_OSC(MATRIX<__int8> &G, DECODING_INFO &decoding_info)
{
	...
	do {
		...
		if ((Pointer.level == message_length) && (Pointer.D_z <= 1)) {
			...
			//
			Best_Goal_2 = Best_Goal;
			A_star_2_stack_OSC(Sorted_G, Metric_Table, Hard_RX,  Pointer, Best_Goal_2, 2, decoding_info);
			Update_Best_Goal_Procedure(Best_Goal_2, Best_Goal, Stack);
			//
			if (Best_Goal.metric < metric_thr)	break;
