...
void A_star_repeatition1(MATRIX<__int8> &G, DECODING_INFO &decoding_info)
{
	size_t
...
void Place_Node_rep(vector<NODE_PATH> &Stack, NODE_PATH &child_node, DECODING_INFO &decoding_info)//把node按照順序放入stack
{
	...
	for (size_t j(0); j < Stack_Size; ++j) {
		...
		if (Stack.at(j).auxiliary() > child_node.auxiliary()) {	//arrange stack by original metric
			position_number = j;
			break;
		}
	}
...
bool Update_Best_Goal_Procedure_rep(NODE_PATH &Child_Node, NODE_PATH &Best_Goal, list<NODE_PATH> &Stack)
{
	if ((Child_Node.auxiliary()) < Best_Goal.metric) {
		Best_Goal = Child_Node; 
		list<NODE_PATH>::iterator  cur = Stack.begin();

		while (cur != Stack.end()) {
			if (cur->auxiliary() > Best_Goal.metric) {
				Stack.erase(cur, Stack.end());
				break;
			}
			cur++;
		}
...
void Place_Node_rep(list<NODE_PATH> &Stack, NODE_PATH &child_node, DECODING_INFO &decoding_info)//把node按照順序放入stack
{
	size_t
		Stack_Size(Stack.size());
	list<NODE_PATH>::iterator  cur = Stack.begin();
	while (cur != Stack.end()) {
		// which is depend on the corresponding value of path metric to the position of new node.
		if (cur->auxiliary() > child_node.auxiliary()) {	//arrange stack by original metric
			break;
		}
		decoding_info.COM++;
		cur++;
	}
	Stack.insert(cur, child_node);
	// If the current capacity of stack is larger than the configured stack size
	// , then delete the last node which stores in the stack.
	if (Stack_Size > decoding_info.StackSize) Stack.pop_back(); // 整個stack的大小為STACK Size +1 
	...
	do {
		...
		for (__int8 new_bit(0); new_bit < 2; ++new_bit) {
			...
			if ((Child_Node.level == message_length) && (Child_Node.auxiliary() < Best_Goal.metric)) {
				...
				//
				Child_Node.heuristic = 0;
				Update_Best_Goal_Procedure_rep(Child_Node, Best_Goal, Stack);
			}
			else if ((Child_Node.level < message_length) && (Child_Node.auxiliary() < Best_Goal.metric)) {
				Place_Node_rep(Stack, Child_Node, decoding_info);
			}
