...
void A_star_RSS_Test(MATRIX<__int8> &G, DECODING_INFO &decoding_info)
{
	...
	int D_z(0);
	for (size_t i(0); i < message_state_number; ++i) {		//計算minimum distance
		//給定 message sequence
...
void A_star_PC_out_Dynamic_CBC_OSC(MATRIX<__int8> &G, DECODING_INFO &decoding_info)  // 修正部分狀況沒有套用DM-I的問題
{
	size_t
		message_length(G.Row_number),
		codeword_length(G.Col_number),
